...
from StormCast.config import MIN_VELOCITY_THRESHOLD, MAX_VELOCITY_THRESHOLD, MOTION_SMOOTHING_WINDOW, BlendingWeights

CHI2_95 = 5.991
LEAD_TIMES = {900: '15 min', 1800: '30 min', 2700: '45 min', 3600: '60 min'}

def evaluate_single_file(file_path, mode="stormcast"):
    results = [] # list of (lt, hit, mae)
    try:
                ...
                for lt in LEAD_TIMES.keys():
                    ...
                    for j in range(i + 1, len(cell_data)):
                        ...
                        if abs(elapsed - lt) <= 150:
                            ...
                            results.append((lt, hit, dist_err))
                            break
...
def run_benchmark(mode, file_list):
    ...
    # lt -> {hits, mae_sum, count}
    stats = {lt: {'hits': 0, 'mae_sum': 0.0, 'count': 0} for lt in LEAD_TIMES.keys()}
    for file_res in all_results:
        for lt, hit, mae in file_res:
            stats[lt]['hits'] += hit
            stats[lt]['mae_sum'] += mae
            stats[lt]['count'] += 1
...
def main():
    ...
    print("\n" + "="*80)
    print(f"{'Mode':<12} | {'Lead':<8} | {'Hit Rate':<10} | {'MAE (km)':<10}")
    print("-" * 80)
    for mode in modes:
        for lt in sorted(LEAD_TIMES.keys()):
            s = benchmarks[mode][lt]
            if s['count'] > 0:
                print(f"{mode.upper():<12} | {LEAD_TIMES[lt]:>7} | {s['hits']/s['count']:>9.1%} | {s['mae_sum']/s['count']:>9.2f}")
        print("-" * 80)
    print("="*80)
