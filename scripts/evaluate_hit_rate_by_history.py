...
def main():
    ...
    print(f"Evaluating EVERYTHING: {len(file_list)} tracks...")
    ...
    for lt in sorted(LEAD_TIMES.keys()):
        ...
        total_evals = sum(stats[lt][h]['total'] for h in range(2, MAX_HISTORY_LEN + 1))
        all_maes = []
        for h in range(2, MAX_HISTORY_LEN + 1): all_maes.extend(stats[lt][h]['mae'])
        all_sigmas = []
        for h in range(2, MAX_HISTORY_LEN + 1): all_sigmas.extend(stats[lt][h]['sigmas'])
        
        if total_evals > 0:
            avg_hits = total_hits / total_evals
            print(f"{LEAD_TIMES[lt]:>7} | {avg_hits:>9.1%} | {np.mean(all_maes):>9.2f} | {np.mean(all_sigmas):>14.2f}")
    print("-" * 75)
    ...
    for lt, label in LEAD_TIMES.items():
        hit_rates = [stats[lt][h]['hits']/stats[lt][h]['total'] if stats[lt][h]['total'] > 0 else np.nan for h in valid_histories]
        ax1.plot(valid_histories, hit_rates, label=label, color=colors[lt], linewidth=2.0, marker='.', markersize=8)
